---
title: Python06-进程和线程1
date: 2020-03-20 20:28:50
tags:
- python
categories:
- python学习
...
mutiprocessing模块提供一个Porcess类来代表一个进程对象，下面例子演示了启动一个子进程并等待其结束：
...
对`Pool`对象调用join()方法会等待所有子进程执行完毕，调用join()之前必须先调用close()，调用close()之后就不能继续添加新的Process了。
